...
class Hkdf {
  ...
  constructor(hmacAlgo) {
    this.hmacAlgo = hmacAlgo
  }

  extract(ikm, salt) {
    var prk = crypto.createHmac(this.hmacAlgo, salt)
      .update(ikm)
      .digest();
  ...
  expand(prk, len, info, nbIterations) {
    var okm = Buffer.alloc(len);
    var t = Buffer.from("");
    for (var i=0; true; i++) {
      t = crypto.createHmac(this.hmacAlgo, prk)
          .update(Buffer.concat([t, info, Buffer.from([i+1])]))
          .digest();
       // Expand
       if (t.copy(okm, i * t.length) < t.length) break;
    }

    return okm;
  }

  digest(ikm, len, salt, info) {
    ...
    var prk = this.extract(ikm, salt);
    var okm = this.expand(prk, len, info, Math.ceil(len / hashLen));
...
function test(test_vectors) {
    for (var i=0; i < test_vectors.length; i++) {
        var test_vector = test_vectors[i];
        var result = new Hkdf(test_vector.algo).digest(
